...
class OperLogService:
    ...
    def info_info(self, info_id: int) -> dict:
        ...
        try:
            ...
            logger.error(f'[查询操作日志记录信息]异常, info_id: {info_id}', exc_info=True)
        return res_data
    ...
    def del_info(self, info_ids: list[int]) -> int:
        try:
            ...
            logger.error(f'[删除操作日志记录信息]异常, info_ids: {info_ids}', exc_info=True)
            return 0
    ...
    def update_info(self, user_id: int, user_name: str, info: dict) -> tuple:
        try:
            ...
            logger.error(f'[更新操作日志记录异常],req_dict:{info}', exc_info=True)
            return -1, '更新操作日志记录异常'
